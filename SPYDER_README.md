...
Download the repository using **Code → Download ZIP**, then extract it into one folder on your PC. Keep all Python files and
`requirements.txt` together. No GitHub account or manuscript file is needed.

## Quick test

After installing the dependencies below, open **quick_test.py** and run the whole
file with F5. A successful run prints `PASS` and verifies three example models on
two held-out wells. It does not overwrite the included results.
